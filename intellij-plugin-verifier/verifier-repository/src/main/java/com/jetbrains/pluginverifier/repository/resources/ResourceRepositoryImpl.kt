...
class ResourceRepositoryImpl<R : Any, K : Any, W : ResourceWeight<W>>(
  ...
  override fun cleanup() {
    ...
    var skipped = 0
    while (true) {
      val stop = myCleanupState.lock.withLock {
        if (myCleanupState.running) {
          myCleanupState.condition.await()
        }
        if (!myCleanupState.running) {
          myCleanupState.running = true
          skipped = myCleanupState.skipped
          true
        } else {
          false
        }
    ...
    try {
      ...
      myCleanupState.lock.withLock {
        myCleanupState.running = false
        myCleanupState.skipped -= skipped
        myCleanupState.condition.signalAll()
      }
    }
  }
  ...
  fun maybeCleanup(): Boolean {
    val skipped: Int = myCleanupState.lock.withLock {
      if (!myCleanupState.running) {
        myCleanupState.running = true
        myCleanupState.skipped
      } else {
        myCleanupState.skipped++
        return false
      }
  ...
  private val myCleanupState = CleanupState()
