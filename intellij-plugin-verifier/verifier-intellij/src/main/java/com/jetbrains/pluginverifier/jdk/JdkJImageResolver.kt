...
class JdkJImageResolver(jdkPath: Path, override val readMode: ReadMode) : Resolver {
  ...
  private val classNameToModuleName: Map<BinaryClassName, String>
  ...
  override fun resolveClass(className: BinaryClassName): ResolutionResult<ClassNode> {
    ...
    if (moduleName != null) {
      val classPath = modulesPath.resolve(moduleName).resolve("$className.class")
      return readClass(className, classPath)
    }
    return ResolutionResult.NotFound
  }

  private fun readClass(className: BinaryClassName, classPath: Path): ResolutionResult<ClassNode> =
    try {
  ...
  private fun readClassNode(className: BinaryClassName, classFilePath: Path): ClassNode =
  ...
  override fun containsClass(className: BinaryClassName) = className in classNameToModuleName
