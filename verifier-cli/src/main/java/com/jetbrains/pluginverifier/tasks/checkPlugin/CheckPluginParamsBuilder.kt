...
class CheckPluginParamsBuilder(val pluginRepository: PluginRepository) : TaskParametersBuilder {
  ...
  private fun getPluginsToCheck(pluginToTestArg: String, ideVersions: List<IdeVersion>): List<PluginInfo> {
    when {
      pluginToTestArg.startsWith("@") -> {
        ...
        return ideVersions.flatMap {
          getPlugins(pluginListFile, it, pluginPaths)
        }
      }
      pluginToTestArg.matches("#\\d+".toRegex()) -> {
        ...
        return createLocalPluginInfo(file).singletonOrEmpty()
      }
    }
  }
      ...
      pluginPaths
          .flatMap {
            if (it.startsWith("id:")) {
              ...
              return createLocalPluginInfo(pluginFile).singletonOrEmpty()
            }
          }
