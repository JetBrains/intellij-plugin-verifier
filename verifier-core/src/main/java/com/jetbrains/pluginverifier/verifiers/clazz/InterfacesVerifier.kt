  ...
  override fun verify(clazz: ClassNode, resolver: Resolver, ctx: VContext) {
    for (o in clazz.interfaces) {
      ...
      if (!VerifierUtil.isInterface(node)) {
        ctx.registerProblem(IncompatibleClassChangeProblem(iface, IncompatibleClassChangeProblem.Change.INTERFACE_TO_CLASS), ProblemLocation.fromClass(clazz.name))
      }
    }
  }
}
