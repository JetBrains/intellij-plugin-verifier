...
class InheritFromFinalClassVerifier : ClassVerifier {
  override fun verify(clazz: ClassNode, resolver: Resolver, ctx: VContext) {
    ...
    val supClass = VerifierUtil.resolveClassOrProblem(resolver, superClassName, clazz, ctx, { ClassLocation(clazz.name) }) ?: return
    if (VerifierUtil.isFinal(supClass)) {
      ctx.registerProblem(InheritFromFinalClassProblem(ClassReference(supClass.name)), ClassLocation(clazz.name))
    }
  }
}
