...
class CheckPlugin(val runnerParams: CheckPluginRunnerParams,
  ...
  override fun computeResult(progress: Progress): CheckPluginResults {
    try {
      ...
      val pluginCoordinates = pluginFiles.map { PluginCoordinate.ByFile(it) }
      val ideDescriptors = ideFiles.map { IdeCreator.createByFile(it, null) }
      val params = CheckPluginParams(pluginCoordinates, ideDescriptors, jdkDescriptor, emptyList(), ProblemsFilter.AlwaysTrue, Resolver.getEmptyResolver(), BridgeVProgress(progress))
