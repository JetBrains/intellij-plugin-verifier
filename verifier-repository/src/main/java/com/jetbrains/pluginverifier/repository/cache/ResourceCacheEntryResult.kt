...
sealed class ResourceCacheEntryResult<R> {
   ...
   * the resource is used in order to release the associated lock in the [ResourceCache].
   */
  data class Found<R>(val resourceCacheEntry: ResourceCacheEntry<R>,
                      val fetchTime: Duration) : ResourceCacheEntryResult<R>()

  /**
   * The resource cache [entry] [ResourceCacheEntry] was not
   * [fetched] [ResourceCache.getResourceCacheEntry]
