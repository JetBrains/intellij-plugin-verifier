...
public class PluginManagerImpl extends PluginManager {
   ...
   */
  @Nullable
  private PluginImpl loadDescriptor(@NotNull final File file, @NotNull String filePath, @NotNull Validator validator) {
    ...
    if (file.isDirectory()) {
      ...
      plugin = loadDescriptorFromZipOrJarFile(file, filePath, validator, PluginXmlExtractor.DEFAULT_PLUGIN_XML_PATH_RESOLVER);
    } else {
      if (!file.exists()) {
    ...
    if (plugin != null) {
      resolveOptionalDescriptors(file, filePath, plugin, validator);
    }
  ...
  private void resolveOptionalDescriptors(@NotNull File file,
                                          ...
                                          @NotNull Validator parentValidator) {
    ...
    if (!optionalConfigs.isEmpty()) {
      ...
      JDOMXIncluder.PathResolver pathResolver = new PluginXmlExtractor.PluginXmlPathResolver(metaInfBases);
      ...
      for (Map.Entry<PluginDependency, String> entry : optionalConfigs.entrySet()) {
        ...
        if (xmlPair != null) {
          ...
          if (url != null) {
            Document document;
            try {
              document = PluginXmlExtractor.resolveXIncludes(xmlPair.getSecond(), url, pathResolver);
  ...
  private PluginImpl loadDescriptorFromEntry(@NotNull ZipEntry entry,
                                             ...
                                             @NotNull Supplier<InputStream> entryStreamSupplier) {
  ...
  private PluginImpl loadFromZipStream(@NotNull final ZipInputStream zipStream,
    ...
    try {
      ...
      while ((entry = zipStream.getNextEntry()) != null) {
        ...
        PluginImpl inRoot = loadDescriptorFromEntry(entry, filePath, zipRootUrl, validator, pathResolver, new Supplier<InputStream>() {
          @Override
  ...
  private PluginImpl loadDescriptorFromZipOrJarFile(@NotNull final File file,
    ...
    PluginImpl descriptorInner = null;
    ...
    try {
      ...
      JDOMXIncluder.PathResolver innerLibJarsResolver = new PluginXmlExtractor.PluginXmlPathResolver(inLibMetaInfUrls);
      ...
      for (final ZipEntry entry : entries) {
        ...
        if (entry.getName().indexOf('/') == -1 && entry.getName().endsWith(".jar")) {
          ...
          PluginImpl plugin = loadFromZipStream(inRootJar, inRootJarUrl, filePath, validator.ignoreMissingFile(), pathResolver);
          if (plugin != null) {
        ...
        PluginImpl inRoot = loadDescriptorFromEntry(entry, filePath, zipRootUrl, validator.ignoreMissingFile(), pathResolver, new Supplier<InputStream>() {
          @Override
          public InputStream get() {
            try {
        ...
        if (LIB_JAR_REGEX.matcher(entry.getName()).matches()) {
          ...
          PluginImpl innerDescriptor = loadFromZipStream(inner, "jar:" + zipRootUrl + entry.getName() + "!/", filePath, validator.ignoreMissingFile(), innerLibJarsResolver);
  ...
  @Nullable
  private PluginImpl loadDescriptorFromDir(@NotNull final File dir, @NotNull String filePath, @NotNull Validator validator) {
  ...
  @Nullable
  private PluginImpl loadDescriptorFromLibDir(@NotNull final File dir, @NotNull String filePath, @NotNull Validator validator) {
    ...
    for (final File f : files) {
      if (isJarOrZip(f)) {
        plugin = loadDescriptorFromZipOrJarFile(f, filePath, validator.ignoreMissingFile(), pathResolver);
        if (plugin != null) {
          //is it necessary to check that only one META-INF/plugin.xml is presented?
          break;
        }
      } else if (f.isDirectory()) {
        PluginImpl descriptor1 = loadDescriptorFromDir(f, filePath, validator.ignoreMissingFile());
        if (descriptor1 != null) {
          if (plugin != null) {
            validator.onIncorrectStructure("Multiple META-INF/" + filePath + " found");
            return null;
  ...
  @NotNull
  @Override
  public PluginCreationResult createPlugin(@NotNull File pluginFile, boolean validatePluginXml) {
