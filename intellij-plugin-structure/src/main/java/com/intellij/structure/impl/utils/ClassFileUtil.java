...
public class ClassFileUtil {
  ...
  private static final int V1_14 = 58;
  ...
  public static String extractFromBytes(byte[] bytes) {
    ...
    if (readShort(bytes, 6) > V1_14) { //hope it will be enough until Java 14 :)
      throw new IllegalArgumentException();
    }
    // parses the constant pool
    final int items[] = new int[readUnsignedShort(bytes, 8)];
    int n = items.length;
    ...
    return readClass(bytes, index + 2, c, strings, items);
  }
